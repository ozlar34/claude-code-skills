...
**Status:** This page is the **pattern walkthrough**. The skill itself is now fully runnable at [`source/github-polish`](../source/github-polish): a portable `gh`-only **core** (everything below except the rendering, zero deps beyond an authenticated `gh`) plus an **optional rendering add-on** (`render/`) that produces the branded social card / banner / diagram via Playwright + Chromium against a neutral default brand spec. The add-on is opt-in by design — the core can't break in your environment; the headless-browser layer is something you add only if you want the branded look (see *Reusable patterns* #9). What stays private is only *my* specific brand (accent choices, glyphs) and the bespoke digest-trick output template — the mechanism is all in source. This very repo's social card and banner were produced by it.
